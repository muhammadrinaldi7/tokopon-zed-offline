            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <nav class="flex text-sm text-gray-500 mb-2">
                        <a href="{{ route('zoffline.reporting') }}" wire:navigate class="hover:text-blue-600">Pusat
                            Laporan</a>
            ...
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6 flex flex-wrap gap-4 items-end">
                <div class="w-full md:w-auto">
                    ...
                    <select wire:model.live="dateRange"
                        class="w-full md:w-48 bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2.5">
                    ...
                    <div class="w-full md:w-auto flex items-center gap-2">
                        <div>
                            ...
                            <input type="date" wire:model.live="startDate"
                                class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2.5 w-36">
                        </div>
                ...
                <div class="w-full md:w-auto flex-1">
                    <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Cari Kasir / Penerima /
                        Order</label>
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.500ms="search"
                            class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5"
                            placeholder="Cari nama atau order...">
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-bold text-gray-800">{{ number_format($summary['total_settlements']) }}
                        </h3>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-bold text-gray-800">Rp
                            {{ number_format($summary['total_tunai'], 0, ',', '.') }}</h3>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-bold text-gray-800">Rp
                            {{ number_format($summary['total_settle'], 0, ',', '.') }}</h3>
                    </div>
                </div>

                <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden group">
                    <div
                        class="absolute right-0 top-0 w-24 h-24 {{ $summary['total_selisih'] < 0 ? 'bg-red-50' : ($summary['total_selisih'] > 0 ? 'bg-orange-50' : 'bg-gray-50') }} rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110">
                    </div>
                    <div class="relative">
                        <p class="text-sm font-medium text-gray-500 mb-1">Total Selisih</p>
                        <h3
                            class="text-3xl font-bold {{ $summary['total_selisih'] < 0 ? 'text-red-600' : ($summary['total_selisih'] > 0 ? 'text-orange-500' : 'text-gray-800') }}">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($settlements as $s)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td
                                        class="px-6 py-4 text-right font-bold {{ $s->total_selisih < 0 ? 'text-red-500' : ($s->total_selisih > 0 ? 'text-orange-500' : 'text-gray-500') }}">
