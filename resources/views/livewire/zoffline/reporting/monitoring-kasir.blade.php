        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p class="text-neutral-500 text-sm mt-1">Rekapitulasi transaksi kasir (Hari Ini) - Cabang:
                        {{ $branchName }}</p>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50 border-b border-gray-100">
                                <th class="py-4 px-6 text-sm font-semibold text-gray-600">Nama FL</th>
                                ...
                                <th class="py-4 px-6 text-sm font-semibold text-gray-600 text-right">Status (Selisih)
                                </th>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-10 h-10 rounded-full bg-blue-100/50 flex items-center justify-center text-blue-600 font-bold">
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <button wire:click="openModalDetail({{ $data['kasir_id'] }})"
                                            class="px-4 py-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 font-semibold text-sm rounded-xl transition-colors">
                                            Lihat Detail
                                        </button>
                                    </td>
                                </tr>
                ...
                <div class="p-6 overflow-y-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($kasirOrders as $order)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="py-3 px-4 text-right font-semibold">Rp
                                        {{ number_format($order['nominal_tunai'], 0, ',', '.') }}</td>
                                    ...
                                    <td class="py-3 px-4 text-right">
                                        @if ($order['settlement'])
                                            <div class="text-sm text-gray-600 font-medium">
                                                Settle: Rp
                                                {{ number_format($order['settlement']['nominal_settle'], 0, ',', '.') }}
                                            </div>
                                            @if ($order['settlement']['selisih'] != 0)
                                                <div class="text-xs text-red-500">
                                                    Selisih: Rp
                                                    {{ number_format($order['settlement']['selisih'], 0, ',', '.') }}
                                                </div>
                                            @endif
                                        @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-8 text-center text-gray-500">Tidak ada transaksi tunai
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
