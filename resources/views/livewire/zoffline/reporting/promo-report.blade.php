<div class="p-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Gunakan laporan ini untuk menagih subsidi promo ke pihak Brand (Principal).</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <button wire:click="exportCsvClaim" wire:loading.attr="disabled"
                ...
                <svg wire:loading.remove wire:target="exportCsvClaim" class="w-4 h-4" fill="none" stroke="currentColor"
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Cabang</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Produk Terjual</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Promo</th>
                    </tr>
                </thead>
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-400 mt-0.5">Inv: {{ $order->accurate_invoice_no ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="flex flex-col gap-1">
                                    @foreach($order->promos as $promo)
                                        <div class="text-sm font-medium text-purple-600 bg-purple-50 px-2 py-1 rounded">
                                            {{ $promo->name }} (Total Rp {{ number_format($promo->pivot->discount_applied, 0, ',', '.') }})
                                        </div>
                                    @endforeach
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <span class="text-gray-500 font-medium">Tidak ada data klaim promo di rentang tanggal ini.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
