<div class="p-6">
                    ...
                    @if ($item->inspections->count() > 0)
                        <div class="p-4 bg-neutral-50 border-t border-neutral-100">
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-3">
                                ...
                                <span class="text-[11px] text-neutral-400">Klik ✏️ untuk koreksi IMEI typo sebelum sync</span>
                            </div>
                            <div class="flex flex-wrap gap-2.5">
                                @foreach ($item->inspections as $ins)
                                    @if ($editingInspectionId === $ins->id)
                                        <!-- Inline Edit Form -->
                                        <div class="inline-flex flex-col bg-white border-2 border-blue-400 rounded-xl p-2 shadow-md animate-fade-in">
                                            <div class="flex items-center gap-1.5">
                                                <input type="text" wire:model="editingImei" wire:keydown.enter="saveEditImei" wire:keydown.escape="cancelEditImei"
                                            ...
                                            @if ($editErrorMessage)
                                                <div class="text-[10px] font-bold text-rose-600 mt-1 max-w-xs leading-tight">{{ $editErrorMessage }}</div>
                                            @endif
                                        </div>
                                    @else
                                        @php
                                        ...
                                        <div
                                            ...
                                            @if ($isLuhnValid)
                                                <span class="px-1.5 py-0.5 mr-1 text-[9px] font-bold bg-emerald-100 text-emerald-700 rounded" title="Luhn Checksum Terverifikasi (15 Digit OK)">15✓</span>
                                            @endif

                                            @if (!$ins->is_pushed)
                                                <button type="button" wire:click="startEditImei({{ $ins->id }})"
    ...
    @if ($activeItemNo && !$scannedImei)
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-900/60 backdrop-blur-sm p-4 overflow-y-auto"
            ...
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative animate-fade-in-up border border-neutral-100">
                <!-- Modal Header Strip -->
                ...
                <div class="p-6 md:p-8 text-center">
                    <div class="flex items-center justify-center gap-2 mb-3">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold border border-blue-200/60">
                    ...
                    <p class="text-xs text-neutral-500 mt-1 mb-4">Gunakan kamera HP atau ketik manual dengan verifikasi otomatis</p>

                    <!-- Target Product Card -->
                    <div class="bg-slate-50 border border-slate-200/80 p-4 rounded-2xl text-left shadow-sm mb-4">
                        <div class="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-1">Target Produk</div>
                        ...
                        <div class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-white rounded-md border border-neutral-200 text-xs font-mono font-bold text-neutral-700">
                            SKU: {{ $activeItemNo }}
                        </div>
                    </div>

                    <!-- Camera Viewfinder if Active -->
                    <div x-show="cameraActive" class="mb-4 relative rounded-2xl overflow-hidden bg-black border-2 border-blue-500 shadow-inner" style="display: none;">
                        ...
                        <div class="p-3 bg-neutral-900 text-white flex items-center justify-between text-xs">
                            <span class="flex items-center gap-1.5 text-emerald-400 font-semibold animate-pulse">
                                <span class="w-2 h-2 rounded-full bg-emerald-400"></span> Kamera Aktif... Arahkan ke Barcode Dus/HP
                            </span>
                            <button type="button" @click="stopCamera()" class="px-3 py-1 bg-neutral-700 hover:bg-neutral-600 rounded-lg text-xs font-bold text-white transition-colors">
                                Tutup Kamera
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 mb-4">
                        <button type="button" @click="if(!cameraActive) startCamera(); else stopCamera();"
                            ...
                            :class="cameraActive ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-blue-50/80 hover:bg-blue-100 text-blue-700 border-blue-200'">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button type="button" @click="stopCamera(); $refs.imeiInput.focus();"
                            class="py-2.5 px-3 rounded-xl border text-xs font-bold transition-all flex items-center justify-center gap-2"
                            :class="!cameraActive ? 'bg-slate-800 text-white border-slate-800 shadow-sm' : 'bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200'">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="relative mb-2 text-left">
                        <input type="text" wire:model="barcodeInput" wire:keydown.enter="processScan"
                            ...
                            :class="{
                                'border-emerald-500 focus:ring-4 focus:ring-emerald-100 bg-emerald-50/20': (rawInput || '').replace(/\D/g, '').length === 15 && checkLuhn(rawInput) === true,
                                'border-rose-400 focus:ring-4 focus:ring-rose-100 bg-rose-50/20': (rawInput || '').replace(/\D/g, '').length === 15 && checkLuhn(rawInput) === false,
                                'border-blue-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100': (rawInput || '').replace(/\D/g, '').length !== 15
                            }"
                    ...
                    <div class="space-y-2 mb-5">
                        <div class="flex items-center justify-between text-xs px-1 min-h-[24px]">
                            <div class="flex items-center gap-1.5">
                                <template x-if="(rawInput || '').replace(/\D/g, '').length === 15 && checkLuhn(rawInput) === true">
                                    <span class="text-emerald-700 font-bold flex items-center gap-1 bg-emerald-100 px-2.5 py-0.5 rounded-md">
                                ...
                                <template x-if="(rawInput || '').replace(/\D/g, '').length === 15 && checkLuhn(rawInput) === false">
                                    <span class="text-rose-700 font-bold flex items-center gap-1 bg-rose-100 px-2.5 py-0.5 rounded-md animate-pulse">
                                ...
                                <template x-if="(rawInput || '').length > 0 && (rawInput || '').replace(/\D/g, '').length !== 15">
                                    <span class="text-slate-500 font-medium bg-slate-100 px-2 py-0.5 rounded-md font-mono text-[11px]">
                            ...
                            <template x-if="(rawInput || '').length >= 4">
                                <div class="text-[11px] font-mono text-slate-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-200">
                                    Akhiran: <strong class="text-blue-700 font-black text-xs" x-text="(rawInput || '').slice(-4)"></strong>
                                </div>
                            </template>
                        </div>
                    </div>
                        ...
                        <div class="px-4 py-2.5 bg-slate-100 border-b border-gray-200 flex items-center justify-between text-xs font-bold text-slate-700">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" @click="showDetailSns = !showDetailSns"
                                    ...
                                    <span x-text="showDetailSns ? 'Sembunyikan Audit IMEI' : '🔍 Lihat Audit Detail IMEI'"></span>
                                </button>
                                <span class="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-200">{{ $toPush }} Unit Siap Sync</span>
                            </div>
                        </div>
                            ...
                            @foreach ($po->items as $item)
                                ...
                                @if ($itemToPush > 0)
                                    <div class="p-4 hover:bg-slate-50 transition-colors">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="text-right">
                                                ...
                                                @if ($unpushedInspections->count() > 0)
                                                    <span class="text-[10px] text-blue-600 font-medium">({{ $unpushedInspections->count() }} SN/IMEI)</span>
                                                @else
                                        ...
                                        @if ($unpushedInspections->count() > 0)
                                            <div x-show="showDetailSns" class="mt-2.5 pt-2 border-t border-slate-100 space-y-1" style="display: none;">
                                                <div class="text-[10px] font-bold text-slate-400 uppercase">Daftar IMEI untuk {{ $item->item_name }}:</div>
                                                <div class="flex flex-wrap gap-1.5">
                                                    @foreach ($unpushedInspections as $ins)
                                                        @php
                                                            $val = \App\Livewire\Zoffline\Inbound\Scan::validateImeiOrSn($ins->imei);
                                                        @endphp
                                                        <span class="inline-flex items-center gap-1 font-mono text-[11px] px-2 py-0.5 rounded border {{ $val['valid'] ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-rose-50 border-rose-200 text-rose-800' }}">
                                                            <span>{{ $ins->imei }}</span>
                                                            @if ($val['valid'] && $val['type'] === 'imei_valid')
                                                                <span class="text-[9px] font-bold text-emerald-600">✓ 15</span>
                                                            @endif
                                                        </span>
                                                    @endforeach
                                                </div>
