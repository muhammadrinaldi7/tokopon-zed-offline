<div class="min-h-screen bg-gray-50 p-4 md:p-8">
    <div class="max-w-3xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <button type="button" wire:click="goBack" class="text-sm font-bold text-gray-500 hover:text-blue-600 inline-flex items-center gap-1 transition-colors">
        ...
        @if (!$foundItem)
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-10 text-center">
                ...
                <p class="text-sm text-gray-500 mb-8">Masukkan SN/IMEI barang yang baru dibeli pelanggan untuk melakukan pengecekan Unboxing QC.</p>

                <form wire:submit="searchItem" class="max-w-md mx-auto">
                    <div class="relative">
                        <input type="text" wire:model="searchQuery" class="w-full bg-gray-50 border-gray-200 rounded-xl px-4 py-3 text-lg font-mono text-center focus:ring-blue-500 focus:border-blue-500" placeholder="Scan atau ketik SN..." autofocus>
                        @error('searchQuery') <span class="text-xs text-rose-500 mt-2 block">{{ $message }}</span> @enderror
                    </div>
                    
                    @if ($errorMessage)
                        <div class="mt-4 p-3 bg-rose-50 text-rose-700 text-sm font-medium rounded-lg border border-rose-100">
                            {{ $errorMessage }}
                        </div>
                    @endif

                    <button type="submit" class="w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors shadow-sm shadow-blue-500/30 flex items-center justify-center gap-2">
                        <svg wire:loading wire:target="searchItem" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        Cari Transaksi
                    </button>
                </form>
            </div>
        @elseif (!$isInspecting)
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-start gap-4 mb-6">
                    <div class="w-12 h-12 rounded-lg bg-emerald-50 text-emerald-500 flex items-center justify-center flex-shrink-0">
                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-gray-900">Transaksi Ditemukan</h2>
                        <p class="text-sm text-gray-500">Pastikan data pelanggan dan barang di bawah ini sesuai sebelum melakukan QC Unboxing.</p>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-xl p-4 space-y-3">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-bold text-gray-900">{{ $foundItem->order->invoice_number }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Tanggal Pembelian</p>
                            <p class="font-medium text-gray-900">{{ $foundItem->order->created_at->format('d M Y H:i') }}</p>
                        </div>
                        <div class="col-span-2 border-t border-gray-200 pt-3">
                            ...
                            <p class="text-sm text-gray-500">{{ $foundItem->order->user->phone_number ?? '-' }}</p>
                        </div>
                        <div class="col-span-2 border-t border-gray-200 pt-3">
                            <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Produk (SN: <span class="font-mono text-gray-900">{{ $foundItem->serial_number }}</span>)</p>
                ...
                <div class="mt-8 flex gap-3">
                    <button type="button" wire:click="goBack" class="flex-1 px-4 py-3 bg-white border border-gray-200 text-gray-700 rounded-xl font-bold hover:bg-gray-50 transition">Batal</button>
                    <button type="button" wire:click="startInspection" class="flex-1 px-4 py-3 bg-emerald-500 text-white rounded-xl font-bold hover:bg-emerald-600 transition shadow-sm shadow-emerald-500/30">Mulai QC Unboxing</button>
                </div>
            </div>
        @elseif ($isSaved)
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div class="mb-6 pb-4 border-b border-gray-100 flex justify-between items-end">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">SN: <span class="font-mono font-bold text-gray-800">{{ $foundItem->serial_number }}</span></p>
                    </div>
                </div>
                ...
                <div x-data="{ qcStep: 0 }" class="relative">
                    ...
                    <div class="mb-6 relative">
                        <div class="h-2 bg-neutral-100 rounded-full overflow-hidden">
                            <div class="h-full bg-blue-500 transition-all duration-300" :style="'width: ' + ((qcStep / {{ $maxQcStep }}) * 100) + '%'"></div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                                @foreach ($slots as $key => $label)
                                    ...
                                    {{ $hasError ? 'border-rose-300 bg-rose-50/20' : 'border-neutral-200 hover:border-neutral-300' }}">
                                        ...
                                        @else
                                            ...
                                            <label class="absolute inset-0 flex flex-col items-center justify-center p-3 text-center select-none overflow-hidden cursor-pointer z-10">
                                                <input type="file" accept="image/*" capture="environment" wire:model="{{ $propertyName }}" class="hidden">
                                                <div class="absolute inset-0 flex items-center justify-center z-0 group-hover:scale-110 transition-transform duration-300">
                                                    <img src="{{ asset('assets/png/' . $key . '.png') }}" alt="{{ $label }}" class="w-16 h-auto object-contain drop-shadow-sm {{ $hasError ? 'opacity-20' : 'opacity-30' }}" onerror="this.onerror=null; this.src='{{ asset('assets/png/default.png') }}';">
                                                </div>
                                    ...
                                    @error('photo_' . $key)
                                        <span class="text-[9px] font-bold text-rose-500 uppercase mt-1 text-center block">{{ $message }}</span>
                                    @enderror
                                @endforeach
                            </div>
                        </div>
                    ...
                    @foreach($categories as $index => $categoryName)
                        <div x-show="qcStep === {{ $index + 1 }}" x-transition.opacity style="display: none;" class="space-y-6">
                            <h4 class="text-lg font-black text-blue-700 uppercase tracking-wider border-b border-neutral-100 pb-2">
                    ...
                    <div x-show="qcStep === {{ $maxQcStep }}" x-transition.opacity style="display: none;" class="space-y-6">
                        <h4 class="text-lg font-black text-blue-700 uppercase tracking-wider border-b border-neutral-100 pb-2">Kesimpulan & Konfirmasi</h4>
                        <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                            <p class="text-sm font-bold text-blue-900">Periksa Kembali Catatan Anda</p>
                            <p class="text-xs text-blue-700 mt-1">Pastikan foto dan checklist QC telah diisi dengan akurat sesuai kondisi perangkat saat diserahterimakan kepada pelanggan.</p>
                        </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center mt-8 pt-4 border-t border-neutral-100 gap-4">
                        ...
                        <div class="w-full md:w-auto flex justify-end">
                            <button x-show="qcStep === 0" type="button" @click="qcStep++" {{ $isPhotoValid ? '' : 'disabled' }}
