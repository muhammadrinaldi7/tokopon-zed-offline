@if ($showManualDiscountModal)
    <div class="fixed inset-0 z-100 flex items-center justify-center">
        ...
        <div class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm transition-opacity"
            wire:click="closeManualDiscountModal">
        </div>

        <!-- Modal Content -->
        <div
            class="relative bg-white rounded-2xl shadow-xl w-full max-w-3xl mx-4 overflow-hidden flex flex-col max-h-[90vh]">
            <!-- Header -->
            ...
            <div class="p-6 overflow-y-auto">
                @php
                    ...
                    $itemBrandId =
                        $manualDiscountCartIndex !== null ? $cart[$manualDiscountCartIndex]['brand_id'] ?? null : null;
                ...
                @if ($validPresets->count() > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        @foreach ($validPresets as $preset)
                            @php
                                $isActive =
                                    isset($cart[$manualDiscountCartIndex]['discount_amount']) &&
                                    $cart[$manualDiscountCartIndex]['discount_amount'] == $preset->amount;
                            @endphp
                            <button
                                wire:click="toggleManualDiscount({{ $manualDiscountCartIndex }}, {{ $preset->amount }})"
                                class="flex items-center justify-between w-full py-2 px-6 rounded-xl border-2 transition-all {{ $isActive ? 'border-indigo-600 bg-indigo-50/50' : 'border-gray-100 hover:border-indigo-200 hover:bg-gray-50' }}">
                                <div class="flex items-baseline gap-1.5">
                                    <span class="text-xs font-semibold text-gray-500">Rp</span>
                                    <span
                                        class="text-lg font-bold {{ $isActive ? 'text-indigo-700' : 'text-gray-900' }}">
                                        {{ number_format($preset->amount, 0, ',', '.') }}
                                    </span>
                                </div>
                                @if ($isActive)
                                    <div
                                        class="w-5 h-5 rounded-full bg-indigo-600 flex items-center justify-center text-white shrink-0 ml-2">
