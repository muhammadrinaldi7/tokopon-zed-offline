    ...
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            ...
            <button wire:click="resetChat"
                class="px-4 py-2 bg-red-200 hover:bg-red-300 text-red-800 rounded-lg text-sm font-medium transition-colors">
                Reset Chat
            </button>
        </div>
        ...
        <div class="bg-white border border-neutral-200 rounded-xl flex flex-col overflow-hidden h-[600px] shadow-sm">
            <!-- Chat Area -->
            <div id="chat-container" class="flex-1 overflow-y-auto p-4 space-y-4 bg-neutral-50" x-data="{ scrollToBottom() { this.$el.scrollTop = this.$el.scrollHeight; } }"
                x-init="scrollToBottom()" @message-sent.window="setTimeout(() => scrollToBottom(), 100)">
                @forelse($histories as $history)
                    @if ($history->role === 'user')
                        <div class="flex justify-end">
                            <div
                                class="bg-indigo-600 text-white max-w-[80%] rounded-2xl rounded-tr-sm px-4 py-3 shadow-sm">
                    ...
                    @else
                        <div class="flex justify-start">
                            <div
                                class="bg-white border border-neutral-200 text-neutral-800 max-w-[80%] rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm">
                ...
                <div wire:loading wire:target="sendMessage" class="flex justify-start w-full">
                    <div
                        class="bg-white border border-neutral-200 text-neutral-500 max-w-[80%] rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm flex items-center space-x-2 mt-4">
                ...
                <form wire:submit="sendMessage" class="flex items-end space-x-2">
                    <div class="flex-1">
                        <textarea wire:model="message" wire:keydown.enter.prevent="sendMessage" rows="1"
                            class="w-full rounded-xl border-neutral-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 resize-none py-3 px-4 text-sm"
                            placeholder="Tanyakan sesuatu tentang database... misal: Berapa jumlah transaksi bulan ini?" required></textarea>
                    </div>
                    <button type="submit" wire:loading.attr="disabled"
                        class="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-4 py-3 flex items-center justify-center transition-colors shadow-sm disabled:opacity-50">
