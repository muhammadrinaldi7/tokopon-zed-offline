<div>
    ...
    @if($this->inspections->isEmpty())
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <p class="text-gray-500 mt-2">Tidak ditemukan data inspeksi untuk IMEI: <span class="font-bold">{{ $imei }}</span></p>
        </div>
    @else
        @php
            $firstInspection = $this->inspections->first();
            $variant = $firstInspection->variant;
            $productName = $variant ? ($variant->secondProduct->name ?? 'Unknown Product') . ' - ' . $variant->storage . ' ' . $variant->color : 'Unknown Product';
        @endphp
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
                    ...
                    <div class="relative pl-3 border-l-2 border-emerald-100 space-y-6">
                        @foreach($this->inspections as $index => $qc)
                            <div class="relative">
                                <div class="absolute -left-[1.1rem] top-1 w-4 h-4 rounded-full border-4 border-white bg-emerald-500 shadow-sm"></div>
                                ...
                                <div class="pl-2">
                                    <div class="flex justify-between items-start mb-1">
                                        <h4 class="text-sm font-bold text-gray-800">
                                            ...
                                            @if($qc->label)
                                                <span class="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded text-xs ml-1">{{ $qc->label }}</span>
                                            @endif
                                        </h4>
                                        <span class="text-[10px] text-gray-400 font-medium">{{ $qc->inspected_at->format('d M Y, H:i') }}</span>
                                    </div>
                                    <p class="text-xs text-gray-500 mb-2">Oleh: <span class="font-semibold text-gray-700">{{ $qc->inspector->name ?? 'Unknown' }}</span></p>
                                    
                                    <div class="flex items-center gap-3">
                                        <div class="px-2 py-1 bg-gray-50 border border-gray-200 rounded text-xs font-bold text-gray-600">
                                            {{ $qc->summary_label }}
                                        </div>
                                        <div>
                                            @if($qc->verdict === 'pass')
                                                <span class="px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-[10px] font-bold uppercase tracking-wider">Pass</span>
                                            @elseif($qc->verdict === 'fail')
                                                <span class="px-2 py-1 bg-rose-100 text-rose-700 rounded text-[10px] font-bold uppercase tracking-wider">Fail</span>
                                            @else
                                                <span class="px-2 py-1 bg-amber-100 text-amber-700 rounded text-[10px] font-bold uppercase tracking-wider">Conditional</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row gap-4 items-center justify-between">
                        <div class="flex-1 w-full">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Bandingkan QC 1 (Kiri)</label>
                            <select wire:model.live="selectedQc1Id" class="w-full rounded-lg border-gray-300 text-sm focus:ring-emerald-500 focus:border-emerald-500 shadow-sm">
                        ...
                        <div class="hidden sm:flex flex-col items-center justify-center pt-5">
                            <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-400 font-bold text-xs shadow-inner">
                                VS
                            </div>
                        </div>
                        
                        <div class="flex-1 w-full">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Dengan QC 2 (Kanan)</label>
                            <select wire:model.live="selectedQc2Id" class="w-full rounded-lg border-gray-300 text-sm focus:ring-emerald-500 focus:border-emerald-500 shadow-sm">
                                <option value="">-- Pilih --</option>
                                @foreach($this->inspections as $idx => $qc)
                                    <option value="{{ $qc->id }}">QC #{{ $this->inspections->count() - $idx }} ({{ $qc->inspected_at->format('d M y') }}) - {{ $qc->label ?? 'No Label' }}</option>
                                @endforeach
                    ...
                    @if($this->qc1 || $this->qc2)
                        @php
                            ...
                            if ($this->qc1 && is_array($this->qc1->checklist_results)) {
                                foreach ($this->qc1->checklist_results as $item) $checklistNames->push($item['name']);
                            }
                            if ($this->qc2 && is_array($this->qc2->checklist_results)) {
                                foreach ($this->qc2->checklist_results as $item) $checklistNames->push($item['name']);
                            }
                            $checklistNames = $checklistNames->unique()->values();
                            
                            // Helper to find item value
                            $findItem = function($qc, $name) {
                                if (!$qc || !is_array($qc->checklist_results)) return null;
                                return collect($qc->checklist_results)->firstWhere('name', $name);
                            };
                            
                            $renderItem = function($item) {
                                if (!$item) return '<span class="text-gray-300">-</span>';
                                ...
                                return '<span class="text-sm font-medium text-gray-700">' . htmlspecialchars($item['value']) . '</span>';
                            };
                        @endphp
                                ...
                                <thead class="text-xs text-gray-500 uppercase bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-center font-semibold border-l border-gray-100 w-1/3">
                                            @if($this->qc1) 
                                                QC Kiri <br><span class="text-[10px] text-gray-400 normal-case">{{ $this->qc1->inspected_at->format('d/m/y') }}</span>
                                            @else - @endif
                                        </th>
                                        <th class="px-6 py-3 text-center font-semibold border-l border-gray-100 w-1/3">
                                            @if($this->qc2) 
                                                QC Kanan <br><span class="text-[10px] text-gray-400 normal-case">{{ $this->qc2->inspected_at->format('d/m/y') }}</span>
                                            @else - @endif
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($checklistNames as $name)
                                        @php
                                            ...
                                            $isChanged = false;
                                            if ($item1 && $item2) {
                                                if ($item1['type'] === 'boolean' && $item1['value'] !== $item2['value']) $isChanged = true;
                                                if ($item1['type'] === 'text' && $item1['value'] !== $item2['value']) $isChanged = true;
                                            }
                                        @endphp
                                        <tr class="hover:bg-gray-50/50 transition">
                                            <td class="px-6 py-2.5 font-medium text-gray-700">
                                                {{ $name }}
                                                @if($isChanged)
                                                    <span class="inline-block ml-2 w-2 h-2 bg-amber-400 rounded-full" title="Berubah"></span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-2.5 text-center border-l border-gray-100 {{ $isChanged ? 'bg-amber-50/30' : '' }}">
                                                {!! $renderItem($item1) !!}
                                            </td>
                                            <td class="px-6 py-2.5 text-center border-l border-gray-100 {{ $isChanged ? 'bg-amber-50/30' : '' }}">
                                                {!! $renderItem($item2) !!}
                                            ...
                                            @if($this->qc1 && $this->qc1->hasMedia('qc_photos'))
                                                <div class="grid grid-cols-2 gap-2">
                                                    @foreach($this->qc1->getMedia('qc_photos') as $media)
                                                        <a href="{{ $media->getUrl() }}" target="_blank" class="block aspect-square rounded-lg overflow-hidden border border-gray-200 hover:border-emerald-500 transition shadow-sm">
