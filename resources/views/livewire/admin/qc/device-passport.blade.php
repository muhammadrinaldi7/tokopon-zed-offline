<div class="relative min-h-screen bg-neutral-50 p-4 sm:p-8 font-sans">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4">
                <a href="{{ route('zoffline') }}" wire:navigate
                    class="w-12 h-12 rounded-2xl bg-white shadow-sm border border-neutral-200 flex items-center justify-center text-neutral-600 hover:bg-neutral-50 hover:text-emerald-600 transition">
                ...
                <div>
                    ...
                    <p class="text-sm text-neutral-500 mt-1 font-medium">Riwayat Inspeksi Fisik & Perbandingan Kualitas
                        Unit</p>
                </div>
            </div>

            <button wire:click="openQcModal"
                class="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-600/30 transition-all flex items-center justify-center gap-2">
        ...
        @if ($this->inspections->isEmpty())
            <div
                class="bg-white rounded-3xl shadow-xl shadow-neutral-200/50 border border-neutral-100 p-12 text-center max-w-2xl mx-auto mt-12">
                <div
                ...
                <p class="text-neutral-500 font-medium">Tidak ditemukan data inspeksi untuk IMEI <br><strong
                        class="text-neutral-800 text-lg mt-2 inline-block bg-neutral-100 px-3 py-1 rounded-lg">"{{ $imei }}"</strong>
                </p>
            </div>
        @else
            @php
            ...
            {{-- Device Info Card --}}
            <div
                class="bg-gradient-to-br from-indigo-900 to-indigo-700 rounded-3xl shadow-2xl p-6 sm:p-8 text-white mb-8 relative overflow-hidden">
                ...
                <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <div class="text-indigo-200 text-[10px] font-black uppercase tracking-widest mb-1">Nomor
                                Seri / IMEI</div>
                            <h2 class="text-2xl sm:text-3xl font-black font-mono tracking-wider text-white">
                                {{ $imei }}</h2>
                            <p
                                class="text-indigo-100 text-sm mt-1 font-medium bg-white/10 inline-block px-3 py-1 rounded-lg">
                                {{ $productName }}</p>
                        </div>
                    </div>
                    <div class="text-left sm:text-right">
                        <div class="text-indigo-200 text-[10px] font-black uppercase tracking-widest mb-1">Total Riwayat
                        </div>
                        <div class="text-3xl font-black text-white">
                            {{ $this->inspections->count() }} <span
                                class="text-sm font-bold text-indigo-300 uppercase tracking-wider ml-1">Inspeksi</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {{-- Comparison Section --}}
            <div
                class="bg-white rounded-3xl shadow-xl shadow-neutral-200/50 border border-neutral-100 overflow-hidden mb-8">
                ...
                <div class="p-6 border-b border-neutral-100 bg-neutral-50/50">
                    <div class="flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div class="flex flex-col items-center justify-center shrink-0">
                            <div
                                class="w-12 h-12 rounded-full bg-neutral-900 text-white flex items-center justify-center font-black text-lg shadow-lg border-4 border-white z-10">
                                VS
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <label
                                class="block text-xs font-black text-emerald-500 uppercase tracking-wider mb-2 text-right">QC
                                Terbaru (Kanan)</label>
                            <select wire:model.live="selectedQc2Id"
                                class="w-full bg-neutral-50 border-0 rounded-xl p-3 text-sm font-bold text-neutral-700 focus:ring-2 focus:ring-emerald-500">
                                <option value="">-- Pilih Inspeksi --</option>
                                @foreach ($this->inspections as $idx => $qc)
                                    <option value="{{ $qc->id }}">QC #{{ $this->inspections->count() - $idx }}
                                        ({{ $qc->inspected_at->format('d M y') }})
                ...
                @if ($this->qc1 || $this->qc2)
                    @php
                        ...
                        if (
                            $this->qc1 &&
                            (is_array($this->qc1->checklist_results) || is_string($this->qc1->checklist_results))
                        ) {
                            $items = is_string($this->qc1->checklist_results)
                                ? json_decode($this->qc1->checklist_results, true)
                                : $this->qc1->checklist_results;
                            if (is_array($items)) {
                                foreach ($items as $item) {
                                    $checklistNames->push($item['name'] ?? 'Unknown');
                                }
                            }
                        }
                        if (
                            $this->qc2 &&
                            (is_array($this->qc2->checklist_results) || is_string($this->qc2->checklist_results))
                        ) {
                            $items = is_string($this->qc2->checklist_results)
                                ? json_decode($this->qc2->checklist_results, true)
                                : $this->qc2->checklist_results;
                            if (is_array($items)) {
                                foreach ($items as $item) {
                                    $checklistNames->push($item['name'] ?? 'Unknown');
                                }
                        ...
                        $findItem = function ($qc, $name) {
                            ...
                            $items = is_string($qc->checklist_results)
                                ? json_decode($qc->checklist_results, true)
                                : $qc->checklist_results;
                            if (!is_array($items)) {
                                return null;
                            }
                        ...
                        $renderItem = function ($item) {
                            if (!$item) {
                                return '<span class="text-neutral-300 font-bold">-</span>';
                            }
                        ...
                        <table class="w-full text-sm text-left border-collapse">
                            <thead>
                                <tr>
                                    ...
                                    <th
                                        class="px-6 py-4 bg-emerald-50/30 text-center font-black text-emerald-500 uppercase tracking-widest text-xs border-b border-emerald-100/50 border-l border-neutral-100 w-1/3 shadow-inner">
                                        QC Kanan
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-neutral-100">
                                @foreach ($checklistNames as $name)
                                    ...
                                    @endphp
                                    <tr
                                        class="hover:bg-neutral-50 transition-colors {{ $isChanged ? 'bg-amber-50/20' : '' }}">
                                        <td class="px-6 py-4 font-bold text-neutral-700 flex items-center gap-2">
                                            {{ $name }}
                                            @if ($isChanged)
                                                <span
                                                    class="inline-flex items-center justify-center w-2 h-2 bg-amber-400 rounded-full animate-pulse"
                                                    title="Terdapat Perubahan"></span>
                                            @endif
                                        </td>
                                        <td
                                            class="px-6 py-4 text-center border-l border-neutral-100 {{ $isChanged ? 'bg-amber-50/20' : '' }}">
                                            {!! $renderItem($item1) !!}
                                        </td>
                                        <td
                                            class="px-6 py-4 text-center border-l border-neutral-100 {{ $isChanged ? 'bg-amber-50/20' : '' }}">
                                ...
                                <tr class="border-t-2 border-neutral-100">
                                    ...
                                    <td class="px-6 py-5 border-l border-neutral-100 align-top">
                                        ...
                                        @else
                                            <div
                                                class="p-4 bg-neutral-50 border border-neutral-200 border-dashed rounded-xl text-center text-xs text-neutral-400 font-bold uppercase tracking-wider">
                                                Tidak ada foto</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-5 border-l border-neutral-100 align-top">
                ...
                @else
                    <div class="p-16 text-center">
                        <div
                            class="w-16 h-16 bg-neutral-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-neutral-100">
                            <svg class="w-8 h-8 text-neutral-300" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <p class="text-neutral-500 font-medium text-sm">Pilih minimal 1 riwayat QC dari *dropdown* di
                            atas untuk melihat detail</p>
                    </div>
                @endif
            </div>
        @endif
            ...
            <div class="bg-white rounded-3xl shadow-xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div class="p-5 overflow-y-auto flex-1 bg-neutral-50/50">
                    <div class="mb-5 bg-white p-4 rounded-2xl shadow-sm border border-neutral-100">
                        <label class="block text-xs font-black text-neutral-500 uppercase tracking-wider mb-2">Pilih
                            Label QC</label>
                        <select wire:model="newQcLabel"
                            class="w-full p-3 rounded-xl border-neutral-200 text-sm font-bold text-neutral-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm bg-neutral-50">
