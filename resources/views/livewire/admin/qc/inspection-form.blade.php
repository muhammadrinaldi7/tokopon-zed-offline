        ...
        <form wire:submit.prevent="saveInspection" class="space-y-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Lakukan pengecekan fisik dan fungsi perangkat dengan teliti.
                    </p>
                </div>

                @if ($template)
                    <div
                        class="bg-[#eff6ff] text-[#1c69d4] px-4 py-2 rounded-lg text-xs font-bold border border-[#1c69d4]/20 flex items-center gap-2">
            ...
            {{-- Info Dasar --}}
            <div
                class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="bg-gray-50/80 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <div
                        class="text-xs font-bold px-3 py-1 bg-white border border-gray-200 rounded-lg text-gray-600 shadow-sm">
                                ...
                                <div class="w-1/2 flex justify-end">
                                    @if ($item['type'] === 'boolean')
                                        <label class="flex items-center gap-3 cursor-pointer select-none">
                                            <span
                                                class="text-xs font-bold {{ $item['value'] ? 'text-emerald-600' : 'text-gray-400' }}">
                                                {{ $item['value'] ? 'OK (PASS)' : 'NOT OK' }}
                                            </span>
                                            <div class="relative flex items-center">
                                                <input type="checkbox"
                                                    wire:model.live="checklistResults.{{ $index }}.value"
                                                    class="sr-only peer">
                                                <div
                                                    class="w-14 h-7 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-emerald-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-emerald-500 shadow-inner">
                                                </div>
                                            </div>
                                        </label>
                                    @else
                                        <input type="text"
                                            wire:model.live="checklistResults.{{ $index }}.value"
                                            placeholder="Input nilai (cth: 92%)"
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea wire:model="inspectorNotes" rows="3"
                        placeholder="Tambahkan catatan khusus jika ada kerusakan tertentu..."
