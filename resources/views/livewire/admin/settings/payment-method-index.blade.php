                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="p-4 bg-amber-50 rounded-lg border border-amber-100">
                        <label class="block text-sm font-bold text-amber-900 mb-1">Accurate Bank No <span
                                class="text-rose-500">*</span></label>
                        <p class="text-xs text-amber-700 mb-3">Kode ini harus sama PERSIS dengan No Akun di Buku Besar
                            (Chart of Accounts) Accurate Anda.</p>
                        <input type="text" wire:model="accurate_bank_no"
                            class="w-full p-2 border-amber-200 rounded-lg focus:ring-amber-500 focus:border-amber-500 text-sm font-mono"
                            placeholder="Contoh: 1102.01 atau KAS-CASH">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh]">
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center shrink-0">
                    <div>
                        <h3 class="font-bold text-lg text-gray-900">Kelola Tarif MDR - {{ $selectedPaymentMethodForRates->name }}</h3>
                        <p class="text-xs text-gray-400">Atur tarif MDR khusus kartu debit/kredit/cicilan untuk EDC ini.</p>
                    </div>
                ...
                <div class="p-6 overflow-y-auto flex-1 space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <button wire:click="createRate" class="px-3 py-1.5 bg-[#1c69d4] hover:bg-blue-700 text-white rounded-lg text-xs font-bold flex items-center gap-1">
                    ...
                    <div class="border rounded-xl overflow-hidden bg-white">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y text-sm">
                                    ...
                                    <tr class="hover:bg-gray-50/50">
                                        ...
                                        <td class="p-3 text-center">
                                            <button wire:click="toggleRateActive({{ $rate->id }})"
                                                ...
                                                <span class="inline-block h-3 w-3 transform rounded-full bg-white transition-transform {{ $rate->is_active ? 'translate-x-5' : 'translate-x-1' }}"></span>
                                            </button>
                                        </td>
                                        <td class="p-3 text-right">
                                            <div class="flex items-center justify-end gap-1.5">
                                                <button wire:click="editRate({{ $rate->id }})" class="p-1.5 text-gray-400 hover:text-[#1c69d4] hover:bg-blue-50 rounded-md transition">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                    </svg>
                                                </button>
                                                <button wire:click="deleteRate({{ $rate->id }})" wire:confirm="Hapus tarif ini?" class="p-1.5 text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-md transition">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="p-6 text-center text-gray-400 italic">Belum ada tarif MDR terkonfigurasi.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end shrink-0">
                    <button wire:click="$set('showRatesModal', false)" class="px-5 py-2 text-sm font-bold text-gray-600 bg-white border rounded-lg hover:bg-gray-50 transition">Tutup</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if ($showRateFormModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Nama Tarif / Opsi / Tenor <span class="text-rose-500">*</span></label>
                        <input type="text" wire:model="rateName" class="w-full p-2 border border-gray-200 rounded-lg focus:ring-[#1c69d4] focus:border-[#1c69d4] text-sm" placeholder="Contoh: Debit BCA (Sesama), Cicilan 3 Bulan, dsb.">
                        @error('rateName')
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        <input type="checkbox" wire:model="rateIsActive" id="rateIsActive" class="rounded text-[#1c69d4] focus:ring-[#1c69d4] border-gray-300">
                        <label for="rateIsActive" class="text-sm text-gray-700">Aktifkan tarif ini</label>
                    </div>
                </div>

                <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                    <button wire:click="$set('showRateFormModal', false)" class="px-5 py-2 text-sm font-bold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition">Batal</button>
                    <button wire:click="saveRate" class="px-5 py-2 text-sm font-bold text-white bg-[#1c69d4] rounded-lg hover:bg-blue-700 transition">Simpan Tarif</button>
                </div>
            </div>
        </div>
    @endif
