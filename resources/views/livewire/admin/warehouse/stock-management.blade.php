<div class="p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-200 pb-1">
        ...
        <div class="w-full md:w-80">
            <div class="relative">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari Item No atau Nama Produk..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-200 text-sm text-gray-700">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500 mt-0.5">
                                    Harga Dasar: Rp {{ number_format($product->base_price ?? 0, 0, ',', '.') }} | SN: {{ $product->has_sn ? 'Ya' : 'Tidak' }}
                                </div>
                            </td>
                            @foreach ($warehouses as $warehouse)
                                @php
                                    $wsRecord = $product->warehouseStocks->firstWhere('warehouse_id', $warehouse->id);
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($warehouses) + 2 }}" class="px-6 py-12 text-center text-gray-500">
                                <svg class="w-12 h-12 mx-auto text-gray-300 mb-3" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                                </svg>
                                <div class="font-medium text-gray-600">Tidak ada produk ditemukan</div>
                                <div class="text-xs text-gray-400 mt-1">Coba cari dengan kata kunci lain atau pilih tab unit usaha yang berbeda.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
