<div class="relative min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 p-4 sm:p-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-indigo-700 tracking-tight">Manajemen Jual HP (SellPhone)</h1>
            <p class="text-sm text-slate-500 mt-2 font-medium">Kelola penawaran beli HP bekas dari pelanggan secara terpusat.</p>
        </div>
    </div>
        ...
        <div class="p-5 border-b border-slate-100/50 flex flex-col sm:flex-row gap-4 bg-white/40">
            <div class="relative flex-1 group">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors duration-300 group-focus-within:text-blue-600">
                    <svg class="w-5 h-5 text-slate-400 group-focus-within:text-blue-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </div>
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari pelanggan atau tipe HP..." class="w-full pl-11 pr-4 py-3 text-sm bg-white/60 border-slate-200/60 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 placeholder:text-slate-400">
            </div>
            <select wire:model.live="status" class="py-3 px-4 text-sm bg-white/60 border-slate-200/60 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 w-full sm:w-56 font-medium text-slate-700 cursor-pointer">
                    ...
                    @forelse ($sellPhones as $item)
                        <tr class="hover:bg-blue-50/30 transition-colors duration-200 group">
                            <td class="px-6 py-5 whitespace-nowrap">
                                <span class="font-bold text-slate-900 bg-slate-100/80 px-2.5 py-1 rounded-md text-xs tracking-wide group-hover:bg-blue-100 group-hover:text-blue-700 transition-colors">#SPL-{{ $item->id }}</span>
                            ...
                            <td class="px-6 py-5">
                                <div class="font-bold text-blue-700 bg-blue-50/50 inline-block px-2.5 py-1 rounded-lg border border-blue-100/50">{{ $item->phone_brand }} {{ $item->phone_model }}</div>
                                <div class="text-[11px] text-slate-500 mt-2 font-medium bg-slate-50 inline-block px-2 py-0.5 rounded border border-slate-100">
                            ...
                            <td class="px-6 py-5 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="px-3 py-1.5 text-[10px] font-black uppercase rounded-lg border tracking-wider {{ $statusColors[$item->status] ?? 'bg-slate-100 text-slate-800 border-slate-200' }} shadow-sm">
                            ...
                            <td class="px-6 py-5 text-right">
                                <a href="{{ route('admin.sell-phones.show', $item) }}" wire:navigate class="inline-flex items-center gap-1.5 px-4 py-2 bg-white text-blue-600 border border-blue-100 shadow-sm hover:shadow hover:bg-blue-50 font-bold text-xs rounded-xl transition-all duration-200 focus:ring-2 focus:ring-blue-500/20">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-sm mt-1 text-slate-500 font-medium">Belum ada pelanggan yang mengajukan penjualan HP di cabang ini.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
