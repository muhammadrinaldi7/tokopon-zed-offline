<div class="relative min-h-screen bg-gradient-to-br p-4 sm:p-8" wire:poll.30s>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.sell-phones.issues') }}" wire:navigate
                ...
                @if (($summary['open_issues_count'] ?? 0) > 0)
                    <span
                        class="px-2 py-0.5 rounded-full bg-rose-600 text-white text-[10px] font-extrabold animate-pulse">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div wire:click="setTab('PAYING')"
            class="cursor-pointer bg-white rounded-2xl p-5 shadow-sm border {{ $summary['paying_count'] > 0 ? 'border-rose-300 ring-4 ring-rose-50/50' : 'border-slate-100 hover:border-blue-200' }} transition-all relative overflow-hidden group">
            ...
            <div class="relative z-10 flex items-center justify-between">
                <div>
                    <p
                        class="text-[11px] uppercase tracking-widest font-bold {{ $summary['paying_count'] > 0 ? 'text-rose-600' : 'text-slate-500' }}">
                        Perlu Dicairkan</p>
                    <h3
                        class="text-3xl font-black {{ $summary['paying_count'] > 0 ? 'text-rose-700' : 'text-slate-800' }} mt-1">
                        {{ $summary['paying_count'] }}</h3>
                    <p
                        class="text-xs font-bold {{ $summary['paying_count'] > 0 ? 'text-rose-500' : 'text-slate-400' }} mt-1">
                        Total: Rp {{ number_format($summary['paying_total'], 0, ',', '.') }}</p>
                </div>
                <div
                    class="w-12 h-12 rounded-full {{ $summary['paying_count'] > 0 ? 'bg-rose-100 text-rose-600' : 'bg-slate-50 text-slate-400' }} flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        <div wire:click="setTab('PENDING_APPROVAL')"
            class="cursor-pointer bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:border-amber-200 transition-all group">
            <div class="flex items-center justify-between">
                ...
                <div
                    class="w-12 h-12 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div wire:click="setTab('INSPECTING')"
            class="cursor-pointer bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:border-indigo-200 transition-all group">
            <div class="flex items-center justify-between">
                <div>
                ...
                <div
                    class="w-12 h-12 rounded-full bg-indigo-50 text-indigo-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div wire:click="setTab('COMPLETED')"
            class="cursor-pointer bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:border-emerald-200 transition-all group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs font-bold text-slate-400 mt-1">Total: Rp
                        {{ number_format($summary['completed_total'], 0, ',', '.') }}</p>
                </div>
                <div
                    class="w-12 h-12 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div
        ...
        <div class="p-5 border-b border-slate-100/50 bg-white/40">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                <button wire:click="setTab('')"
                    class="px-4 py-2 text-xs font-bold rounded-full border transition-all {{ $status === '' ? 'bg-slate-800 text-white border-slate-800 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50' }}">
                    Semua
                </button>
                <button wire:click="setTab('PAYING')"
                    class="relative px-4 py-2 text-xs font-bold rounded-full border transition-all {{ $status === 'PAYING' ? 'bg-rose-500 text-white border-rose-500 shadow-sm' : 'bg-white text-rose-600 border-rose-200 hover:bg-rose-50' }}">
                ...
                <button wire:click="setTab('PENDING_APPROVAL')"
                    class="px-4 py-2 text-xs font-bold rounded-full border transition-all {{ $status === 'PENDING_APPROVAL' ? 'bg-amber-500 text-white border-amber-500 shadow-sm' : 'bg-white text-amber-600 border-amber-200 hover:bg-amber-50' }}">
                    Approval ({{ $summary['pending_approval_count'] }})
                </button>
                <button wire:click="setTab('INSPECTING')"
                    class="px-4 py-2 text-xs font-bold rounded-full border transition-all {{ $status === 'INSPECTING' ? 'bg-indigo-500 text-white border-indigo-500 shadow-sm' : 'bg-white text-indigo-600 border-indigo-200 hover:bg-indigo-50' }}">
                    Inspeksi ({{ $summary['inspecting_count'] }})
                </button>
                <button wire:click="setTab('COMPLETED')"
                    class="px-4 py-2 text-xs font-bold rounded-full border transition-all {{ $status === 'COMPLETED' ? 'bg-emerald-500 text-white border-emerald-500 shadow-sm' : 'bg-white text-emerald-600 border-emerald-200 hover:bg-emerald-50' }}">
                    Selesai
                </button>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1 group">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search"
                        placeholder="Cari pelanggan, tipe HP, atau #ID..."
                        class="w-full pl-11 pr-4 py-2.5 text-sm bg-white/60 border-slate-200/60 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 placeholder:text-slate-400 font-medium">
                </div>
                <select wire:model.live="date_filter"
                    class="py-2.5 px-4 text-sm bg-white/60 border-slate-200/60 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 w-full sm:w-48 font-bold text-slate-700 cursor-pointer">
                ...
                @if ($status === 'INSPECTING')
                    <select wire:model.live="status_inspeksi"
                        class="py-2.5 px-4 text-sm bg-white/60 border-slate-200/60 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-300 w-full sm:w-48 font-bold text-slate-700 cursor-pointer">
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100/60">
                    @forelse ($sellPhones as $item)
                        ...
                        <tr
                            class="transition-colors duration-200 group {{ $isPaying ? 'bg-amber-50/40' : '' }} {{ $isCompleted ? 'opacity-70' : '' }}">
                            <td
                                class="px-6 py-5 whitespace-nowrap {{ $isPaying ? 'border-l-4 border-l-rose-500' : 'border-l-4 border-l-transparent' }}">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span
                                        class="font-black text-slate-900 bg-slate-100/80 px-2.5 py-1 rounded-md text-xs tracking-wide group-hover:bg-blue-100 group-hover:text-blue-700 transition-colors">#SPL-{{ $item->id }}</span>
                                    @if ($isNew && !$isCompleted)
                                        <span
                                            class="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-rose-100 text-rose-600 border border-rose-200 animate-pulse">Baru</span>
                                    @endif
                                    @if ($item->openIssues->count() > 0)
                                        <span
                                            class="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-amber-100 text-amber-700 border border-amber-200 flex items-center gap-1"
                                            title="Terdapat {{ $item->openIssues->count() }} kendala belum selesai">
                            ...
                            <td class="px-6 py-5">
                                <div class="font-bold text-slate-800 flex items-center gap-1.5 text-xs">
                                    ...
                                    {{ optional($item->branch)->name ?? 'Global' }}
                                </div>
                            ...
                            <td class="px-6 py-5">
                                <div
                                    class="font-bold text-blue-700 bg-blue-50/50 inline-block px-2.5 py-1 rounded-lg border border-blue-100/50 text-xs">
                                    {{ $item->phone_brand }} {{ $item->phone_model }}
                                </div>
                                <div
                                    class="text-[10px] text-slate-500 mt-1.5 font-medium bg-slate-50 inline-block px-2 py-0.5 rounded border border-slate-100">
                            ...
                            <td class="px-6 py-5 text-right whitespace-nowrap">
                                @if ($item->appraised_value)
                                    <div
                                        class="text-base font-black {{ $isPaying ? 'text-rose-600' : 'text-emerald-600' }}">
                                ...
                                <div class="mt-2 flex flex-col items-end gap-1">
                                    <span
                                        class="px-2 py-1 text-[9px] font-black uppercase rounded border tracking-wider {{ $statusColors[$item->status] ?? 'bg-slate-100 text-slate-800 border-slate-200' }} shadow-sm">
                            ...
                            <td class="px-6 py-5 text-center">
                                <span
                                    class="px-2.5 py-1 text-[10px] font-bold rounded-full {{ $agingColor }} inline-block">
                                    {{ $agingText }}
                                </span>
                            </td>
                            <td class="px-6 py-5 text-right">
                                <a href="{{ route('admin.sell-phones.show', $item) }}" wire:navigate
                                    ...
                                    <svg class="w-4 h-4 {{ $isPaying ? 'animate-bounce-x' : '' }}" fill="none"
                            ...
                            <td colspan="7" class="px-6 py-16 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-sm mt-1 text-slate-500 font-medium">Belum ada pelanggan yang
                                        mengajukan penjualan HP di cabang ini dengan filter tersebut.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
