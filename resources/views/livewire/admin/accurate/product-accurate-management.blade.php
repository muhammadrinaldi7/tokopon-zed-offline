<div>
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <button wire:click="syncItems" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="syncItems">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                </span>
                <span wire:loading wire:target="syncItems">
                    <svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                </span>
                Sinkronisasi {{ $activeTab === 'syihab' ? 'Baru' : 'Bekas' }}
            </button>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        <th class="py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Accurate ID / SKU</th>
                        <th class="py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Barang</th>
                        <th class="py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">Harga Dasar</th>
                        <th class="py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center">Stok</th>
                        <th class="py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">Terakhir Sinkron</th>
                    </tr>
                </thead>
                    ...
                    @forelse ($products as $product)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="py-4 px-6 text-right text-xs text-gray-500">
                                ...
                                <div class="text-[10px] text-gray-400 mt-0.5">{{ $product->updated_at->format('d/m/Y H:i') }}</div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="py-12 text-center">
                                ...
                                <p class="text-sm text-gray-400 mt-1">Klik tombol Sinkronisasi untuk menarik data dari database {{ $activeTab === 'syihab' ? 'Baru' : 'Bekas' }}.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
