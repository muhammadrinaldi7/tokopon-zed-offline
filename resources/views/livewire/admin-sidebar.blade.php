<aside :class="sidebarCollapsed ? 'lg:w-20' : 'lg:w-64'"
    ...
    <nav class="flex-1 py-6 px-4 space-y-1.5 overflow-y-auto no-scrollbar overflow-x-hidden">
        ...
        <a href="#"
            class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-colors cursor-pointer {{ request()->is('admin/promos') ? $activeClass : $inactiveClass }}">
            <svg class="w-5 h-5 {{ request()->is('admin/promos') ? $activeIconClass : $inactiveIconClass }}"
                fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
            </svg>
            <span x-show="!sidebarCollapsed" class="whitespace-nowrap transition-opacity">Promo & Voucher</span>
        </a>

        {{-- <a href="#"
            class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-colors cursor-pointer {{ request()->is('admin/tips') ? $activeClass : $inactiveClass }}">
        ...
        @hasanyrole('admin|superadmin')
            ...
            <div x-data="{ openProducts: {{ request()->routeIs('admin.products', 'admin.products.variants', 'admin.categories', 'admin.brands') ? 'true' : 'false' }} }">
                <button @click="openProducts = !openProducts" type="button"
                    class="w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm transition-colors cursor-pointer {{ request()->routeIs('admin.products', 'admin.products.variants', 'admin.categories', 'admin.brands') ? $activeClass : $inactiveClass }}">
