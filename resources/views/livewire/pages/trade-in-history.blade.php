<div class="max-w-7xl mx-auto p-2  md:p-6 min-h-screen">
    ...
    <div class="mb-8 mt-2 flex gap-2  items-center">
        ...
        <p class="text-gray-500 text-xs md:text-sm">Pantau status pengajuan tukar tambah Anda.</p>
    </div>
    <div class="space-y-4">
        @forelse($tradeIns as $item)
            <a href="{{ route('trade-ins.show', $item) }}" wire:navigate
                ...
                <div class="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div class="flex items-center gap-4">
                        <div
                            ...
                            <img src="{{ $item->targetProduct->getFirstMediaUrl('cover', 'thumb') ?: $item->targetProduct->getFirstMediaUrl('gallery', 'thumb') }}"
                                class="object-contain max-h-full max-w-full">
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Barang
                                Ditukar</p>
                            <h3 class="font-bold text-gray-900">{{ $item->old_phone_brand }}
                                {{ $item->old_phone_model }}</h3>
                            <div class="flex items-center gap-2 mt-1">
                                <span class="text-xs text-gray-500">ke: <span
                                        class="font-bold text-[#4E44DB]">{{ $item->targetProduct->name }}</span></span>
                            </div>
                        </div>
                    </div>
        ...
        @empty
            <div
                ...
                <img src="{{ asset('assets/png/tradein.png') }}" alt="" class="w-70 h-auto">
                <h3 class="font-bold text-gray-900 text-lg">Kamu belum punya pengajuan</h3>
                <p class="text-gray-500 mt-1 text-xs md:text-sm">Yuk ajukan tukar tambah sekarang!</p>
