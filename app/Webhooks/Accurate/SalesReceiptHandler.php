...
class SalesReceiptHandler implements WebhookHandlerInterface
{
    public function handle(AccurateWebhookLog $log): void
    {
        ...
        $salesReceiptId = $payload['salesReceiptId'] ?? null;
        $salesReceiptNo = $payload['salesReceiptNo'] ?? null;

        if (!$salesReceiptId && !$salesReceiptNo) {
            Log::warning("Accurate Webhook Sales Receipt Data tidak memiliki salesReceiptId atau salesReceiptNo: " . json_encode($payload));
            return;
        }
        ...
        try {
            ...
            Log::error("Error processing Sales Receipt Webhook: " . $e->getMessage());
        }
    }
}
